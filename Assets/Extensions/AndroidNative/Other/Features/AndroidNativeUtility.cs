...
public class AndroidNativeUtility : SA_Singleton<AndroidNativeUtility> {
	...
	private void RefreshTokenCodeReceived(string data) {
		...
		if (status == 1) {
			...
			AN_RefreshTokenResult result = new AN_RefreshTokenResult (false, "Request Authorization Code error");
	...
	private IEnumerator GenerateRefreshTokenRequest(string code, string clientId, string clientSecret, string redirectUrl) {
		...
		if (string.IsNullOrEmpty (response.error)) {
			...
			AN_RefreshTokenResult result = new AN_RefreshTokenResult (true, access_token, refresh_token, token_type, expiresIn);
			OnOAuthRefreshTokenLoaded (result);
		} else {
	...
	private IEnumerator RefreshOAuthTokenRequest(string clientId, string clientSecret, string refreshToken) {
		...
		if (string.IsNullOrEmpty (response.error)) {
			...
			AN_AccessTokenResult result = new AN_AccessTokenResult (true, access_token, token_type, expiresIn);
	...
	private IEnumerator ObtainUserDeviceCodeRequest(string clientId) {
		...
		if (string.IsNullOrEmpty (response.error)) {
			...
			AN_DeviceCodeResult result = new AN_DeviceCodeResult (true, device_code, user_code, verification_url, expires_in, interval);
